...
import { SeedSkillData } from '../src/seeding/seed-skill.data';
...
describe('RequestsController (E2E with Pre‑seeded Data)', () => {
  ...
  beforeEach(async () => {
    ...
    authTokenR = (
      await authService.login(SeedUserData[1].email, SeedUserData[1].password)
    ).accessToken;

    // Находим навыки для тестирования
    const offeredSkillR = await skillRepository.findOne({
      where: { title: SeedSkillData[0].title },
      relations: ['owner'],
    });
    ...
    if (offeredSkillR) {
      ...
      throw new Error(
        `Offered skill "${SeedSkillData[0].title}" not found in database`,
      );
    }

    const requestedSkillR = await skillRepository.findOne({
      where: { title: SeedSkillData[1].title },
      relations: ['owner'],
    });
    ...
    if (requestedSkillR) {
      ...
      throw new Error(
        `Requested skill "${SeedSkillData[1].title}" not found in database`,
      );
    }
  });
